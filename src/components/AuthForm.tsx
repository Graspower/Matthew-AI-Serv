...
export function AuthForm({ onAuthSuccess }: AuthFormProps) {
  ...
  const handleSendOtp = async (data: PhoneFormData) => {
    setIsLoading(true);
    try {
      // It's a common requirement to prefix with country code. Assuming US for simplicity.
      const formattedPhoneNumber = `+1${data.phone}`; 
  ...
  return (
    <>
                ...
                {!phoneAuthState.isOtpSent ? (
                    <Form {...phoneForm}>
                        <form onSubmit={phoneForm.handleSubmit(handleSendOtp)} className="space-y-6 pt-4">
                                ...
                                <FormItem> 
                                    ...
                                    <FormControl>
                                        <div className="flex items-center gap-2">
                                            <span className="text-sm border h-10 px-3 flex items-center rounded-md bg-muted">+1</span>
                            ...
                            <Button type="submit" className="w-full" disabled={isLoading}> {isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Send OTP </Button>
                        </form>
                    </Form>
                ) : (
                    <Form {...otpForm}>
                        <form onSubmit={otpForm.handleSubmit(handleVerifyOtp)} className="space-y-6 pt-4">
                            ...
                            <Button type="submit" className="w-full" disabled={isLoading}> {isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Verify OTP </Button>
                        </form>
                    </Form>
                )}
            </TabsContent>
        ...
        <Button variant="outline" className="w-full" onClick={handleGoogleSignIn} disabled={isLoading}>
            {isLoading && activeTab !== 'login' && activeTab !== 'signup' ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <GoogleIcon className="mr-2 h-5 w-5" />}
            Google
        </Button>
    </>
  );
